 ...
 */
var launchExperiment = function(exp_id, nodes, image_id, size_id){
   ...
   function(err){
      if(err){
         ...
         console.log("Experiment " + exp_id + " deployed");
         return;
      }
   });
}
...
var _getApplication = function(app_id, cb){
   ...
   collection.findOne({id: app_id}, function(err, app){
      if(err){
         cb(err);
      } else {
         cb(null, app);
...
var _getExperiment = function(exp_id, cb){
   ...
   collection.findOne({id: exp_id}, function(err, exp){
      if(err){
         cb(err);
      } else {
         cb(null, exp);
